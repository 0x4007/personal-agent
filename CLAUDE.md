...
- **Execution Environment**: GitHub Actions ONLY (provides compute and shell access)
- **AI Core**: Claude Code Action (submodule integration)
- **Event Router**: UbiquityOS (captures events from multiple platforms and dispatches to GitHub Actions)
- **Invocation**: Username mentions trigger agent activation via GitHub Actions workflow dispatch
...
- **Backend**: GitHub Actions is the sole backend - no local servers, no cloud workers
...
- `/src/action.ts` - GitHub Actions entry point
- `/src/handlers/` - Event handlers for different commands
- `/src/handlers/claude-agent.ts` - Main Claude CLI integration
- `/src/types/` - TypeScript type definitions
- `/src/index.ts` - Main plugin logic (called by action.ts)
- `/tests/` - Jest test suite with mocks
- `/dist/index.js` - Bundled GitHub Action (generated by build-action.js)
...
4. Install UbiquityOS app on the fork for event routing to GitHub Actions
5. All execution happens in GitHub Actions - no external deployments needed
6. Configure MCP servers for additional platforms

**IMPORTANT**: This agent runs EXCLUSIVELY in GitHub Actions. There is no local development server, no Cloudflare Workers, no external compute. GitHub Actions is the backend.
