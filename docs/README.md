# Personal Agent Documentation

Welcome to the Personal Agent documentation. This universal automation platform acts as your digital assistant across multiple platforms.

## Quick Start

- [Installation Guide](getting-started/installation.md) - Get up and running in minutes
- [Configuration](getting-started/configuration.md) - Set up your credentials and preferences
- [Your First Agent](getting-started/first-agent.md) - Create and deploy your personal agent

## User Guides

- [GitHub Integration](user-guide/github-integration.md) - Automate GitHub workflows
- [Telegram Bot Setup](user-guide/telegram-usage.md) - Connect via Telegram
- [Available Commands](user-guide/commands.md) - Complete command reference
- [Troubleshooting](user-guide/troubleshooting.md) - Common issues and solutions

## Developer Documentation

- [Architecture Overview](developer-guide/architecture.md) - System design and components
- [API Reference](developer-guide/api-reference.md) - Complete API documentation
- [Extending the Platform](developer-guide/extending.md) - Add new platforms
- [MCP Development](developer-guide/mcp-development.md) - Create Model Context Protocol servers
- [Contributing](developer-guide/contributing.md) - How to contribute

## Operations

- [Deployment Guide](operations/deployment.md) - Production deployment procedures
- [Monitoring Setup](operations/monitoring.md) - Observability and metrics
- [Operational Runbook](operations/runbook.md) - Incident response procedures
- [Security Guidelines](operations/security.md) - Security best practices

## API Documentation

- [OpenAPI Specification](api/openapi.yaml) - Machine-readable API spec

## Support

- GitHub Issues: [Report bugs or request features](https://github.com/anthropics/claude-code/issues)
- Documentation: [Claude Code Docs](https://docs.anthropic.com/en/docs/claude-code/)

## License

This project is open source and available under the MIT License.