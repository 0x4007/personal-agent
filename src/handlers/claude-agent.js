...
export async function claudeAgent(context) {
...
Provide a brief but helpful response.`;
        ...
        await context.commentHandler.postComment(context, logger.error(`Failed to process command with Claude: ${error instanceof Error ? error.message : String(error)}`));
    }
    logger.verbose(`Exiting claudeAgent`);
}
function buildPrompt(isReadOnly, context) {
...
async function executeClaudeCommand(prompt, logger, maxRetries = 3) {
    let lastError = null;
...
async function executeClaudeCommandInternal(prompt, logger) {
...
async function configureGitHubAuth(token, logger) {
    try {
