...
function warnOnce(logger, key, message) {
    if (!logger || typeof logger.warn !== "function" || warned.has(key))
        return;
    warned.add(key);
    logger.warn(message);
}
...
export function getVectorDbConfig(logger) {
    const rawUrl = getEnvValue("UOS_VECTOR_DB_URL") ?? getEnvValue("SUPABASE_URL");
    const rawKey = getEnvValue("UOS_VECTOR_DB_KEY") ?? getEnvValue("SUPABASE_SERVICE_ROLE_KEY") ?? getEnvValue("SUPABASE_KEY") ?? getEnvValue("SUPABASE_ANON_KEY");
    ...
    if (!url || !key) {
        warnOnce(logger, "vector-db-missing-config", "Vector DB disabled: missing Supabase URL/key. Set UOS_VECTOR_DB_URL/UOS_VECTOR_DB_KEY or SUPABASE_URL + SUPABASE_*_KEY (or SUPABASE_PROJECT_ID).");
        return null;
    }
    return { url, key };
}
...
export async function fetchVectorDocument(config, nodeId) {
...
export async function fetchVectorDocuments(config, ids, options = {}) {
...
export async function fetchVectorDocumentsByParentId(config, parentId, options = {}) {
...
export async function findSimilarIssues(config, params) {
    if (!params.currentId.trim() || params.embedding.length === 0)
        return [];
...
export async function findSimilarComments(config, params) {
    if (!params.currentId.trim() || params.embedding.length === 0)
        return [];
