...
export class TelegramFormatter implements ResponseFormatter {
  format(text: string): string {
    ...
    return this._escapeMarkdownV2(text);
  }

  formatCode(code: string, language = ""): string {
    // Telegram supports code blocks with language specification
    const escapedCode = this._escapeCodeBlock(code);
    return `\`\`\`${language}\n${escapedCode}\n\`\`\``;
  }

  formatError(error: string): string {
    const escapedError = this._escapeMarkdownV2(error);
    return `❌ *Error*: ${escapedError}`;
  }

  formatList(items: string[]): string {
    return items.map((item) => `• ${this._escapeMarkdownV2(item)}`).join("\n");
  }
  ...
  private _escapeMarkdownV2(text: string): string {
    // Escape special characters for MarkdownV2
    // Order matters: escape backslash first to avoid double-escaping
    return text.replace(/\\/g, "\\\\").replace(/([_*()~`>#+\-=|{}.!])/g, "\\$1");
  }

  private _escapeCodeBlock(code: string): string {
