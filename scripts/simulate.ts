...
function parseArgs(argv: string[]): SimulationOptions {
  const options: SimulationOptions = { issueUrl: "", isPromptOnly: false };
...
function parseIssueUrl(raw: string): IssueRef {
  let url: URL;
  try {
    url = new URL(raw);
  } catch {
...
async function fetchIssueData(ref: IssueRef, token: string | null): Promise<IssueData> {
  if (!token) {
    return { isPr: ref.isPrHint };
  }
  try {
    const url = `https://api.github.com/repos/${ref.owner}/${ref.repo}/issues/${ref.issueNumber}`;
    const issue = (await fetchGithubJson(url, token)) as { pull_request?: unknown; title?: string; html_url?: string };
    return {
...
async function fetchIssueCommentById(ref: IssueRef, commentId: number, token: string): Promise<IssueComment> {
  const url = `https://api.github.com/repos/${ref.owner}/${ref.repo}/issues/comments/${commentId}`;
  return (await fetchGithubJson(url, token)) as IssueComment;
}

function startsWithMention(body: string, agentOwner: string): boolean {
  const trimmed = body.trim().toLowerCase();
  const mention = `@${agentOwner}`.toLowerCase();
  return trimmed.startsWith(mention);
}

async function fetchLatestMentionComment(ref: IssueRef, agentOwner: string, token: string): Promise<IssueComment | null> {
  ...
  const comments = (await fetchGithubJson(url, token)) as IssueComment[];
...
function parseCommandFromBody(body: string, agentOwner: string): string {
...
async function resolveCommentBody(params: {
  ...
  agentOwner: string;
  token: string | null;
}): Promise<{ body: string; actor: string; htmlUrl?: string }> {
